    ...
    <div class="max-w-2xl mx-auto p-6 bg-white rounded shadow-md mt-10">
        ...
        <form action="{{ route('usuarios.update', $usuario->id_usuario) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $usuario->nombre) }}" required class="w-full p-2 border border-gray-300 rounded">
            </div>
            <div class="mb-4">
                <label for="email" class="block text-sm font-medium text-gray-700">Email:</label>
                <input type="email" name="email" id="email" value="{{ old('email', $usuario->email) }}" required class="w-full p-2 border border-gray-300 rounded">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="id_centro" id="id_centro" class="w-full p-2 border border-gray-300 rounded">
                    ...
                    @foreach($centros as $centro)
                        <option value="{{ $centro->id_centro }}" {{ old('id_centro', $usuario->id_centro) == $centro->id_centro ? 'selected' : '' }}>
                            {{ $centro->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex space-x-2">
                <button type="submit" class="bg-teal-600 text-white px-4 py-2 rounded hover:bg-teal-700">Actualizar</button>
